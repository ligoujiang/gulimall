...
import com.pxy.gulimall.coupon.domain.SeckillSession;
import com.pxy.gulimall.coupon.service.ISeckillSessionService;
...
public class SeckillSessionController extends BaseController
{
     ...
     * 查询秒杀活动场次列表
     */
    //@PreAuthorize("@ss.hasPermi('coupon:session:list')")
    @GetMapping("/list")
     ...
     * 导出秒杀活动场次列表
     */
    //@PreAuthorize("@ss.hasPermi('coupon:session:export')")
     ...
     * 获取秒杀活动场次详细信息
     */
    //@PreAuthorize("@ss.hasPermi('coupon:session:query')")
    @GetMapping(value = "/{id}")
     ...
     * 新增秒杀活动场次
     */
    //@PreAuthorize("@ss.hasPermi('coupon:session:add')")
     ...
     * 修改秒杀活动场次
     */
    //@PreAuthorize("@ss.hasPermi('coupon:session:edit')")
     ...
     * 删除秒杀活动场次
     */
    //@PreAuthorize("@ss.hasPermi('coupon:session:remove')")
